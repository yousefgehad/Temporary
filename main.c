...
uint32_t TempValueC_1;
	    uint32_t TempValueF_1 ;
	    uint32_t ADCValues[1];
		Adc_ConfigType Adc_Config_Test;
...
int main(void) {
	...
	CANEnable(CAN0_BASE);
		  //ADC
			ADC_Config_Init(ADC0_BASE,SYSCTL_PERIPH_ADC0);
	    ADC_Init(&Adc_Config_Test);
	...
	while(1) {
		 Adc_EnableHardwareTrigger(ADC0_BASE);
	        ...
	        // Read ADC Value.
						...
						Adc_ReadGroup(ADC0_BASE,ADCValues);
	       // ADCSequenceDataGet(ADC0_BASE, 3, ADCValues);
	        ...
	        TempValueC_1 = (uint32_t)(147.5 - ((75.0*3.3 *(float)ADCValues[0])) / 4096.0);
	        // Get Fahrenheit value.  Make sure you divide last to avoid dropout.
	        TempValueF_1 = ((TempValueC_1 * 9) + 160) / 5;
	        // Display the temperature value on the console.
	        UARTprintf("Temperature = %3d*C or %3d*F\r\n", TempValueC_1,TempValueF_1);
		
		
		msgDataPtr[0] = TempValueC_1;
		msgDataPtr[1] = TempValueC_1;
		msgDataPtr[2] = TempValueC_1;
		msgDataPtr[3] = TempValueC_1; // 50% intensity
		/*
